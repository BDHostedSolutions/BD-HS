...
resource "azurerm_virtual_machine_extension" "kpapp01_iaasantimalware" {
  name                       = "${var.resource_name_prefix}-${var.kpapp01_name}-IaaSAntimalware"
  location                   = "${azurerm_resource_group.rg.location}"
  resource_group_name        = "${azurerm_resource_group.rg.name}"
  virtual_machine_name       = "${azurerm_virtual_machine.KPAPP01.name}"
  ...
  depends_on = ["azurerm_virtual_machine.KPAPP01"]
}
