...
resource "azurerm_virtual_machine" "KPFILE01" {
  ...
  storage_os_disk {
    name              = "${var.resource_name_prefix}-${var.kpfile01_name}_OS"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "${var.resource_name_prefix}-${var.kpfile01_name}"
